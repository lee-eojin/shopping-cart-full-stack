...
> (초안 대비 유지/개정/폐기 이력 — 무엇이 왜 바뀌었는지의 기록).

---

## 1. 개요

- 금액이 결정되는 값(쿠폰 판정·할인액·금액 4종)은 전부 BE가 계산하고 FE는 표시한다.
  FE에는 쿠폰 계산 모듈이 없다.
- 쿠폰 적용·금액 계산의 진입점은 `POST /order/apply` — 검증하고, 계산하고, 결과를
  주문서에 저장한다. 화면을 그리는 원천은 항상 `GET /order`.
...
필드명은 팀 합의 명세 표기를 따른다. 명세에 정의가 빈 부분은 주석으로 표시하고
...
export interface PercentageCoupon extends CouponBase {
  ...
  discountRate: number;                          // 명세에 예시 없음 — 가정 (system-design §5-6)
  availableTime: { start: string; end: string }; // 명세에 예시 없음 — 가정 (system-design §5-6)
}
...
// 적용 불가 사유(reason)는 명세에 없음 — 모달 안내 문구에 필요 (system-design §5-5)
```
...
export interface OrderItem {
  ...
  // 상품명·이미지 없음 — 화면 표시에 필드 추가 또는 상품 목록 조인 필요 (system-design §5-3)
}
...
export interface ApplyCouponsRequest {
  cartItemIds: number[]; // 주문서 항목과의 관계는 가정으로 처리 (system-design §5-1)
  couponIds: Coupon["id"][];
  isRemoteArea: boolean;
}
...
설계 근거(초안에서 확정, 변동 없음): base는 교집합만 둔다 — 기준은 "중요한가"가 아니라
"전 변형이 갖는가"다. optional 범벅은 불가능한 상태를 타입이 허용하게 만들어 `?? 0`이
규칙을 가리고, base에 다 넣고 `Omit`으로 도려내는 방식은 변형이 추가될 때 빠뜨리기 쉽다.
...
- `ctx`(계산 컨텍스트)는 `{ items, orderAmount, isRemoteArea, now }`. `now`는 주입한다 —
...
- 같은 판정 모듈(`couponRules`)을 두 라우트가 재사용한다 — `GET /coupons`(표시·조합표),
  `POST /order/apply`(확정 검증·계산). 모달을 연 뒤 시간이 지나 판정이 달라질 수 있으므로
  apply가 확정 시점에 다시 판정한다. 규칙은 한 곳에 두고 호출만 나눈다.
...
- FE에 couponModel이 없다. 모달의 비활성화, 예상 할인, 화면의 금액 4종은 전부 서버
  응답 필드의 표시다. 토글 중 예상 할인은 `secondPrice` 표에서 선택한 couponIds로
  찾아 읽기만 한다. 정률 쿠폰의 할인액이 동반 쿠폰에 따라 달라지므로 쿠폰별 할인액
  합산으로는 맞는 값이 나오지 않는다.
- `useOrder`: 주문서가 화면의 유일한 데이터원. `useQuery(['order'])`로 구독하고,
  ...
  다이어그램의 "PATCH 후 GET으로 다시 그린다" 흐름이 step 2 뮤테이션 패턴과 같아서
  새 인프라가 필요 없다.
...
- 에러 처리: 기존 `apiRequest`의 errorMessage 정규화 흐름 그대로. apply 거부(400)는
  ...
  `GET /order` 404는 장바구니로 리다이렉트 (404는 가정 — system-design §5-4).
...
초안(작년 스펙 가정) 대비. 변경 이유를 남기기 위한 표다.
...
| 2 | 판정+계산 단일 진입점 `assessCoupon` → union 반환 | 유지 (위치 이동) | 규칙 누수 차단 논리는 그대로. FE 모듈 → BE `couponRules.ts`. 단 명세에 reason 필드가 없어 반환 shape의 사유 부분은 보류 (system-design §5-5). |
| 3 | 최적 조합 = 순서쌍 전수 평가 | 재개정 → 조합 전수 평가 (순서는 규칙 고정) | 스펙 보강으로 "정액 먼저, 정률은 할인된 금액에"가 확정(요구사항 §3.1) — 할인 상호작용이 살아나 2차안(개별 상위 2개 합산)은 무효. 2장 이하 부분집합 10개 전수 평가. 산출물의 이름이 `primaryPrice`(최적)·`secondPrice`(전체 표)로 계약화됐다. |
| 4 | 시간(`now`/클럭) 주입 | 유지 | BE로 옮겨도 동일 — `createCouponRouter(db, clock)`. 경계값 테스트의 전제. |
| 5 | FE = 미리보기 계산 + BE = 주문 시점 권위 | 폐기 → BE 단독 계산 | 스펙 명시 제약 "클라이언트가 임의로 계산하지 않도록". 부수 효과로 절사 정합(#10)·시계 이원화 문제가 함께 사라졌다. |
| 6 | 에러 코드 400/404/500 + 구체적 errorMessage | 유지 | 기존 API 컨벤션·FE 에러 경로 일관. 에러 body 형식은 명세 미정 — `errorMessage`로 가정 (system-design §5-8). |
...
| 10 | 퍼센트 절사 규칙 명문화 | 유지 (이유 변경) | 원래 근거(FE/BE 정합)는 #5 폐기로 사라졌지만, 금액 규칙의 미정의는 계산 주체가 하나여도 문제가 된다 — 명세로 유지. |
| 11 | 모달 토글은 로컬, 합계는 조합표 룩업 | 유지 (계약명 변경: combinations → secondPrice) | 토글 왕복은 로딩·경쟁 상태를 만들고, 쿠폰별 합산은 정률 상호작용 때문에 부정확하며 식 실행은 금지된 계산. 조합표(≤10개) 룩업은 클라 계산이 없다. |
| 12 | 요청에 가격·금액 미포함 (id·수량·플래그만) | 유지 | 서버가 받는 순간 검증 대상이 된다. DB 원본 재조회가 가장 싼 검증이고, "가격·쿠폰의 SSOT는 DB"라는 선언이기도 하다. |
...
| 14 | `POST /order`가 최적 조합을 자동 적용, `GET /order`는 응답 전 재판정 | 폐기 → #7 개정으로 흡수 | 자동 적용 주체가 서버에서 FE 트리거로 바뀌어 생성 시점 자동 적용이 사라짐. GET의 만료 쿠폰 재판정은 명세에 없다 — 시간제 쿠폰의 만료 후 할인 노출 가능성은 구현에서 확인 (§7). |
| 15 | `POST /order/apply`는 계산 결과를 주문서에 저장 | 신설 | 저장하지 않으면 새로고침 시 적용 상태가 사라지고, 도서산간 PATCH 후 재계산도 불가능하다(서버가 적용 쿠폰을 모름). `GET /order`가 쿠폰 할인을 응답하는 것 자체가 저장을 전제. 팀 다이어그램과 일치. |
| 16 | 도서산간 변경은 PATCH 후 `GET /order` 재조회 | 신설 (팀 다이어그램) | 변경 요청(PATCH)과 화면 갱신(GET)의 분리 — 응답 계약이 단순해지고 "화면의 원천은 GET /order"가 모든 흐름에서 동일. FE는 invalidate(['order'])로 자연스럽게 구현된다. |

## 7. 미해결 / 리뷰에서 확인

- 명세 구멍 8건 — [`system-design.md`](./system-design.md) §5가 원본 목록.
  이 문서의 "확인 필요" 주석은 전부 그 목록을 가리킨다 (cartItemIds 관계, 상품명 부재,
  reason 부재, percentage/freeShipping 필드, `minimumAccount` 표기 등).
- 시간제 쿠폰의 만료 후 할인 노출 — 옛 설계의 "GET /order 응답 전 재판정"이 빠지면서,
